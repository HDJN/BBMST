...
class UploadFile {//类定义开始
		...
		try {
			...
			if (isset($exif['IFD0']['Orientation'])) {
				...
				if ($orientation != 0) {
					...
					$this->rotateImg($filename, $degrees);
				}
			}
		} catch (Exception $e) {
		}
	 ...
	 * @param  integer $degrees  所需角度
	 * @return void
	 */
	private function rotateImg ($filename, $degrees) {
		...
		switch ($ext) {
			case "png":
				...
				$rotate = imagerotate($temp, $degrees, 0);
				imagepng($rotate, $filename);
				break;

			case "jpeg":
				$temp = imagecreatefromjpeg($filename);
				$rotate = imagerotate($temp, $degrees, 0);
			...
			case "gif":
				$temp = imagecreatefromgif($filename);
				$rotate = imagerotate($temp, $degrees, 0);
